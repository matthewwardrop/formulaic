...
@pytest.mark.skipif(check_for_pyarrow(), reason="PyArrow is required to run the arrow tests.")
class TestArrowMaterializer:

    @pytest.fixture
    def data(self):
        import pyarrow
        return pyarrow.Table.from_pandas(pandas.DataFrame({'a': [1, 2, 3], 'A': ['a', 'b', 'c']}))
    ...
    def test_state(self, materializer):
        ...
        mm2 = ArrowMaterializer(pyarrow.Table.from_pandas(pandas.DataFrame({'a': [4, 5, 6]}))).get_model_matrix(mm.model_spec)
