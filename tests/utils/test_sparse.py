...
def test_sparse_category_encoding():
    categories = pandas.Series(list('abcdefgabcdefg'))

    encoded = categorical_encode_series_to_sparse_csc_matrix(categories)
    reduced_encoded = categorical_encode_series_to_sparse_csc_matrix(categories, reduced_rank=True)
