    ...
    def _get_coding_matrix(self, levels, reduced_rank=True, sparse=False):
        ...
        for i in range(len(levels) - 1):
            if self.reverse:
                ...
                contr[i, i] = n - i - 1
        contr[numpy.triu_indices(n - 1) if self.reverse else numpy.tril_indices(n, k=-1)] = -1
...
class PolyContrasts(Contrasts):
    ...
    def _get_coding_matrix(self, levels, reduced_rank=True, sparse=False):
        ...
        if self.scores and not len(self.scores) == n:
            raise ValueError("`PolyContrasts.scores` must have the same cardinality as the categories.")
